...
from src.utils_sections import (
    render_preview_html,
    # STRICT schema-driven parsing (kept)
    split_resume_by_schema,
    # NEW: fallback headline-based parsing (must be implemented in utils_sections.py)
...
# Static downloads
# -----------------------------
WORKER_ROOT = Path(__file__).resolve().parents[1]          # worker-py/
OUTPUTS_ROOT = WORKER_ROOT / "outputs"                     # worker-py/outputs
OUTPUTS_ROOT.mkdir(parents=True, exist_ok=True)

# serve: http://<worker>/files/<job_id>/<filename>
app.mount("/files", StaticFiles(directory=str(OUTPUTS_ROOT), html=False), name="files")


def get_generator() -> Optional[ResumeGenerator]:
    """
    Lazy init Gemini ResumeGenerator.
    Uses per-request hard timeout inside core.py (httpx).
    """
    global _generator
...
class Section(BaseModel):
    ...
    # for grouping (front-end expects these)
...
class ParseReq(BaseModel):
    ...
    # allow either inline schema OR schema_path for compatibility
    ...
    # Optional: debug / trace only
    schema_name: Optional[str] = None

    # Optional hint (safe even if omitted): choose fallback mode when no schema
...
class ExportReq(BaseModel):
    job_id: str
    sections: List[OptimizedSection]
    export_pdf: bool = False  # keep for compatibility

    # passed from front-end resumeFile.name
...
def _load_schema_from_path(schema_path: str) -> Dict[str, Any]:
    """
    Load schema JSON from disk. Raise ValueError on invalid JSON or empty content.
    """
    if not schema_path:
...
def parse(req: ParseReq):
    """
    Parsing behavior:
    - If schema (inline) OR schema_path is provided -> STRICT schema-driven parsing (existing behavior).
    - If schema is NOT provided -> fallback headline-based splitting (mainstream CV headings).
    """
    t0 = time.time()
    ...
    try:
        ...
        if isinstance(req.schema, dict) and req.schema:
        ...
        if schema_obj is not None:
            ...
            # Fallback mode (headline splitter)
        ...
        # Normalize to Section model (unchanged behavior)
        sections: List[Section] = []
...
def _build_markdown_from_sections(sections: List[OptimizedSection]) -> str:
    """
    Prefer optimized_text; if empty, fallback to text.
    """
    parts: List[str] = []
...
def _sanitize_base_name(name: str) -> str:
    """
    Keep human-friendly filename base, strip extension, remove unsafe chars.
    """
    n = (name or "").strip()
    if not n:
        return "Resume"

    # If it's a path, keep basename only
    n = os.path.basename(n)

    # Strip last extension (.pdf/.docx/...)
...
def _next_version(out_dir: Path, base: str) -> int:
    """
    Determine next vN by scanning existing files in out_dir:
      <base>_v<number>.docx/.pdf
    """
...
def export(req: ExportReq, request: Request):
    """
    Exports TWO downloadable files with versioned names:
      <OriginalResumeBase>_v1.docx
      <OriginalResumeBase>_v1.pdf
    Next export increments v2, v3, ...
    Served via /files/<job_id>/<filename>
    """
    try:
